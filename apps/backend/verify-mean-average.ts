...
async function verify() {
  try {
    console.log('🔧 Verifying Mean or Average of an Array problem...');
    
    // Delete the old entry
    const deleteResult = await prisma.question.deleteMany({
      where: {
        slug: 'average-of-all-elements-in-an-array-tcs-nqt'
      }
    });
    ...
    const problem = await prisma.question.findUnique({
      where: { slug: 'mean-or-average-of-an-array-tcs-nqt' }
    });
