...
async function verify() {
  console.log('🔧 Verifying Palindromes in a Range problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/find-all-palindrome-numbers-in-a-given-range-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/palindromes-in-a-range-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    console.log('📝 Sample Output (first 50 chars):', problem.sampleOutput?.substring(0, 50));
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        const m = lines[0];
        const n = lines[1];
        const outputPreview = tc.output.length > 30 ? tc.output.substring(0, 30) + '...' : tc.output;
        console.log(`  ${i + 1}. Range [${m}, ${n}] → Output: [${outputPreview}] (visible)`);
      });
