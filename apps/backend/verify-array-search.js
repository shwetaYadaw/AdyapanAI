...
async function verify() {
  console.log('🔧 Verifying Array Search problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/search-an-element-in-an-array-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/array-search-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        const x = tc.input.split('\n')[2];
        console.log(`  ${i + 1}. Array: [${inputArr}], x=${x} → Output: ${tc.output} (visible)`);
      });
