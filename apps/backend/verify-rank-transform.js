...
async function verify() {
  console.log('🔧 Verifying Rank Transform of an Array problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/replace-each-element-of-the-array-by-its-rank-in-the-array-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/rank-transform-of-an-array-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        console.log(`  ${i + 1}. Input: [${inputArr}] → Output: [${tc.output}] (visible)`);
      });
