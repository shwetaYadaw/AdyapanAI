...
async function verify() {
  console.log('🔧 Verifying Primes in a Range problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/prime-numbers-in-a-given-range-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/primes-in-a-range-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        const l = lines[0];
        const r = lines[1];
        const outputPreview = tc.output || '(empty)';
        console.log(`  ${i + 1}. Range [${l}, ${r}] → Output: ${outputPreview} (visible)`);
      });
