...
async function verify() {
  console.log('🔧 Verifying Array after k Rotations problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/finding-circular-rotation-of-an-array-by-k-positions-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/array-after-k-rotations-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    console.log('📝 Sample Input (first 30 chars):', problem.sampleInput?.substring(0, 30));
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        const arr = lines[1];
        const k = lines[2];
        console.log(`  ${i + 1}. Array: [${arr?.substring(0, 20)}], k=${k} → Output: [${tc.output.substring(0, 25)}...] (visible)`);
      });
