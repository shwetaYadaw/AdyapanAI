...
async function verify() {
  console.log('🔧 Verifying Check if a number is prime problem...\n');

  // Check for old problem
  const oldResult = await makeRequest('/api/v1/challenges/questions/check-if-a-number-is-prime-or-not-tcs-nqt');
  
  // Check for new problem
  const newResult = await makeRequest('/api/v1/challenges/questions/check-if-a-number-is-prime-tcs-nqt');
