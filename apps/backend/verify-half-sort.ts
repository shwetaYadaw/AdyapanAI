...
async function verify() {
  try {
    console.log('🔧 Verifying Half Ascending and Half Descending Sort problem...');
    
    // Delete the old entry
    const deleteResult = await prisma.question.deleteMany({
      where: {
        slug: 'rearrange-array-in-increasing-decreasing-order-tcs-nqt'
      }
    });
    ...
    const problem = await prisma.question.findUnique({
      where: { slug: 'half-ascending-and-half-descending-sort-tcs-nqt' }
    });
