...
async function verify() {
  console.log('🔧 Verifying Rotate Array (rotation) problem...\n');
  ...
  const newResult = await makeRequest('/api/v1/challenges/questions/rotate-array-rotation-tcs-nqt');
  ...
  if (newResult.data) {
    ...
    if (problem.testCases) {
      ...
      problem.testCases.slice(0, 3).forEach((tc, i) => {
        ...
        const arr = lines[1];
        const k = lines[2];
        console.log(`  ${i + 1}. Array: [${arr?.substring(0, 20)}...], k=${k} → Output: [${tc.output.substring(0, 30)}...] (visible)`);
      });
