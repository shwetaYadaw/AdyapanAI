...
async function updateTrieWithOldSlug() {
  try {
    // Delete the new one with new slug
    await prisma.question.delete({
      where: { slug: 'implement-trie-prefix-tree' }
    }).catch(() => {}); // Ignore if doesn't exist

    // Create new one with old slug so it's found correctly
    const trieProblem = {
      title: 'Implement Trie (Prefix Tree)',
      slug: 'implement-trie', // Use the old slug for backwards compatibility
...
\`\`\``,
    ...
    console.log('✅ Implement Trie problem updated with correct slug!');
