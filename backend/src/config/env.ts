...
export const env = {
  ...
  DOCKER_SOCKET_PATH: optionalEnv('DOCKER_SOCKET_PATH', process.platform === 'win32' ? '//./pipe/docker_engine' : '/var/run/docker.sock'),
  DOCKER_NETWORK: optionalEnv('DOCKER_NETWORK', 'execution-network'),
  AUTO_CLEANUP: optionalEnv('AUTO_CLEANUP', 'true') === 'true',
  CLEANUP_INTERVAL: parseInt(optionalEnv('CLEANUP_INTERVAL', '300000'), 10),
  DEFAULT_CPU_TIME_LIMIT: parseInt(optionalEnv('DEFAULT_CPU_TIME_LIMIT', '5'), 10),
  DEFAULT_MEMORY_LIMIT: parseInt(optionalEnv('DEFAULT_MEMORY_LIMIT', '256'), 10),
  DEFAULT_MAX_PROCESSES: parseInt(optionalEnv('DEFAULT_MAX_PROCESSES', '128'), 10),
  DEFAULT_MAX_FILE_SIZE: parseInt(optionalEnv('DEFAULT_MAX_FILE_SIZE', '10'), 10),

  // Judge0 (Legacy - Deprecated)
